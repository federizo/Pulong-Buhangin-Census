...
export const HouseProfileDELETE = async (houseProfileId: number): Promise<{ success: boolean; message?: string }> => {
    ...
    try {
        ...
        InsertLogForUpdate("Delete HouseProfile", `Deleted HouseProfile with ID number of ${houseProfileId}`, houseProfileId)
        return { success: true };
    } catch (error) {
...
export const FamMemberDELETE = async (famID: number, houseprofileid: string): Promise<{ success: boolean; message?: string }> => {
    const supabase = await createSupbaseServerClient();
    ...
    try {
        ...
        await InsertLogForUpdate("Delete FamMember", `Deleted FamMember with ID ${famID}`, famID);
        return { success: true };
    } catch (error) {
        console.error("An error occurred during deletion:", error);
        return { success: false, message: error instanceof Error ? error.message : "Unknown error" };
