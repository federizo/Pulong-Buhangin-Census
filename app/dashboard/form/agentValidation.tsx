...
export const agentValidation = async (agentid: string): Promise<{ isValid: boolean; message: string; agentData: any }> => {
    ...
    if (error) {
        ...
        return { isValid: false, message: 'Error during validation', agentData: [] };
    }

    // If no permission data is found, return invalid
    if (!permission) {
        return { isValid: false, message: 'Agent not found or has no permissions', agentData: [] };
    }
