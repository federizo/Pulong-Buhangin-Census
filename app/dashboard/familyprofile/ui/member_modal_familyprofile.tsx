...
const MemberModalFamilyProfile = ({ item, modal, setModal }: { item: any, modal: boolean, setModal: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const orignialData = useRef(selectedUser)
     ...
     const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            const letterOnlyFields = ["FirstName", "LastName", "MiddleName", "Suffix", "Disability", "other", "position"];
    ...
    const handleUpdate = async (item: any, prevItem: any) => {
        ...
        try {
            const response = await updateSingleMember(item, prevItem);
            toast({
                title: response.msg,
            });
            location.reload()
    ...
    const handleDeleteFamilyMember = async () => {
        try {
            setDeleting(true);
            const res = await FamMemberDELETE(selectedUser.MemberId, item.HouseProfileId);
    ...
    <input
        ...
        maxLength={2} // Optional safeguard for UI
    />
)}
                    </div>
                  </div>
